...
void CH_SR04_Init(void)
{
	...
	GPIO_InitStructer.GPIO_Pin=GPIO_Pin_8 |GPIO_Pin_6|GPIO_Pin_10|GPIO_Pin_12;
	GPIO_Init(GPIOB, &GPIO_InitStructer);

	/*ECOH回响信号*/
	GPIO_InitStructer.GPIO_Mode=GPIO_Mode_IN_FLOATING;
	GPIO_InitStructer.GPIO_Pin=GPIO_Pin_9 |GPIO_Pin_7|GPIO_Pin_11|GPIO_Pin_13;
	GPIO_Init(GPIOB, &GPIO_InitStructer);
...
float Senor_Using_harmful(void)
{
...
float Senor_Using_recoverable(void)
{
	...
	while(i!=1)
	{
		...
		i+=1; //每收到一次回响信号+1，收到5次就计算均值
		while(GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_7)==SET);//回响信号消失
		TIM_Cmd(TIM4,DISABLE);//关闭定时器
...
float Senor_Using_kitchen(void)
{
	...
	while(i!=1)
	{
		...
		i+=1; //每收到一次回响信号+1，收到5次就计算均值
		while(GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_11)==SET);//回响信号消失
		TIM_Cmd(TIM4,DISABLE);//关闭定时器
...
float Senor_Using_other(void)
{
	...
	while(i!=1)
	{
		...
		i+=1; //每收到一次回响信号+1，收到5次就计算均值
		while(GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_13)==SET);//回响信号消失
		TIM_Cmd(TIM4,DISABLE);//关闭定时器
